...
class XNORConv2d(Module):
    ...
    def forward(self, x):
        ...
        self.mean_val = (
            self.fp_weights.abs().view(self.out_channels, -1).mean(1, keepdim=True)
        )

        self.conv.weight.data.copy_(
            self.fp_weights.data.sign() * self.mean_val.view(-1, 1, 1, 1)
        )
        x = self.conv(x)
    ...
    def update_gradient(self):
        ...
        self.fp_weights.grad = (
            self.fp_weights.grad
            * self.fp_weights.data[0].nelement()
...
class XNORLinear(Module):
    ...
    def forward(self, x):
        ...
        self.mean_val = (
            self.fp_weights.abs().view(self.out_features, -1).mean(1, keepdim=True)
        )

        self.linear.weight.data.copy_(
            self.fp_weights.data.sign() * self.mean_val.view(-1, 1)
        )
        x = self.linear(x)
        return x
