...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, wbit, abit, stride=1):
        ...
        self.conv3 = conv1x1(
            planes, self.expansion * planes, wbit=wbit, abit=abit, stride=1
        )
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, wbit=1, abit=1, num_classes=10):
        ...
        self.layer1 = self._make_layer(
            block, 64, num_blocks[0], wbit=wbit, abit=abit, stride=1
        )
        self.layer2 = self._make_layer(
            block, 128, num_blocks[1], wbit=wbit, abit=abit, stride=2
        )
        self.layer3 = self._make_layer(
            block, 256, num_blocks[2], wbit=wbit, abit=abit, stride=2
        )
        self.layer4 = self._make_layer(
            block, 512, num_blocks[3], wbit=wbit, abit=abit, stride=2
        )
