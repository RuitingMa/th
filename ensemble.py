...
class Ensemble:
    ...
    def __init__(self,bin_type_1,bin_type_2,dataset,
    train_batch_size_1,train_batch_size_2,test_batch_size, 
    model_1,model_2,device,optimizer,lr,steps,gamma):
        ...
        #models 1-6
        if bin_type_1 == 'xnor':
            classification_1 = XnorClassifier(self.model_1, train_data[0], self.test_loader, self.device)
            classification_2 = XnorClassifier(self.model_1, train_data[1], self.test_loader, self.device)
            classification_3 = XnorClassifier(self.model_1, train_data[2], self.test_loader, self.device)
            classification_4 = XnorClassifier(self.model_1, train_data[3], self.test_loader, self.device)
        ...
        # model 0
        if bin_type_2 == 'xnor':
            classification_0 = XnorClassifier(model_2, train_loader_0, self.test_loader, self.device)
        ...
        if optimizer == 'adam':
            self.optimizer_1 = torch.optim.Adam(self.model_1.parameters(), lr=lr, weight_decay=1e-5)
            self.optimizer_2 = torch.optim.Adam(self.model_2.parameters(), lr=lr, weight_decay=1e-5)
        elif optimizer == 'sgd':
            self.optimizer_1 = torch.optim.SGD(self.model_1.parameters(), lr=lr, momentum=0.9,
                weight_decay=5.e-4)
            self.optimizer_2 = torch.optim.SGD(self.model_2.parameters(), lr=lr, momentum=0.9,
                weight_decay=5.e-4)
        
        self.scheduler_1 = torch.optim.lr_scheduler.MultiStepLR(self.optimizer_1, steps,
            gamma=gamma)
        self.scheduler_2 = torch.optim.lr_scheduler.MultiStepLR(self.optimizer_2, steps,
            gamma=gamma)
    ...
    def train (self,checkpoint,epochs_1,epochs_2):
        ...
        for member in self.expert_classifiers:
            test_accuracy, preds, conf = member.train(self.criterion, self.optimizer_1, 
                epochs_1, self.scheduler_1, checkpoint)

        test_accuracy, preds, conf = self.model0.train(self.criterion, self.optimizer_2, 
            epochs_2, self.scheduler_2, checkpoint)
        preds_0 = preds
    ...
    def test (self,preds_0):
        ...
        preds_ensemble = benn.square_confidence_modified(self.expert_classifiers,
            self.model0.confidences,preds_0)
